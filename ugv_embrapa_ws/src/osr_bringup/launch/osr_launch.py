...
def generate_launch_description():
    ...
    ld.add_action(
        Node(
            package='osr_control',
            executable='roboclaw_wrapper_nm',
    ...
    ld.add_action(
        Node(
            ...
            emulate_tty=True,
            respawn=True,
            parameters=[{'centered_pulse_widths': [30,130,96,80]}]  # pulse width where the corner motors are in their default position, see rover_bringup.md.
        )
    )
    ld.add_action(
